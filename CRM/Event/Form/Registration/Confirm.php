...
class CRM_Event_Form_Registration_Confirm extends CRM_Event_Form_Registration {
  ...
  public function postProcess() {
    ...
    //@erawat modified CPM-183 - copy the main registrant for skiped registrant
    $firstParticipant = NULL;
    foreach ($params as $participantNum => $record) {
      $isFlex = CRM_Flexibleregistrant_Utils::isEventConfiguredToUseFlexiblePriceSet($this->_eventId);
      if($isFlex){
        if($participantNum == 0){
          $firstParticipant = $record;
        }
        if ($record == 'skip') {

          $skipParticipant = $firstParticipant;
          $skipParticipant['first_name'] = $firstParticipant['first_name'] . '_' . $participantNum;
          $skipParticipant['last_name'] = $firstParticipant['last_name'] . '_' . $participantNum;
          $skipParticipant['is_primary'] = 0; //force primary to be zero
          //unset field that not using in the additional participant
          unset($skipParticipant['email-Primary']);
          unset($skipParticipant['additional_participants']);
          unset($skipParticipant['additional_participants']);
          unset($skipParticipant['invoiceID']);
          unset($skipParticipant['MAX_FILE_SIZE']);
          unset($skipParticipant['contact_id']);
          unset($skipParticipant['payment_action']);
          unset($skipParticipant['ip_address']);
          unset($skipParticipant['payment_processor']);
          unset($skipParticipant['defaultRole']);
          $skiptLineItem = array();
          $priceFields = civicrm_api("PriceField","get", array ('version' => '3', 'price_set_id' => $firstParticipant['priceSetId']));
          if(CRM_Utils_Array::value('count', $priceFields) == 1){
            $priceFieldId = CRM_Utils_Array::value('id', $priceFields);
            $priceField = CRM_Utils_Array::value('id', $priceFields['values'][$priceFieldId]);
            ...
            foreach ($priceFieldValue['values'] as $key => $value) {
              if( CRM_Utils_Array::value('amount', $value) == 0 && CRM_Utils_Array::value('is_default', $value) == 1){
                $priceFieldValueId = CRM_Utils_Array::value('id', $value);
                $skipParticipant['amount_level'] = CRM_Utils_Array::value('label', $value);
                $skipParticipant['amount'] = CRM_Utils_Array::value('amount', $value);
                $skipParticipant['amount_priceset_level_radio'] = array($priceFieldValueId =>  CRM_Utils_Array::value('label', $value));
                $skipParticipant['price_123'] = array($priceFieldValueId => 1);

                //Hacked - added lineItem on fly when create a participant
                $skiptLineItem['price_field_id'] = $priceFieldId;
                $skiptLineItem['price_field_value_id'] = $priceFieldValueId;
                $skiptLineItem['label'] =  CRM_Utils_Array::value('label', $value);
                $skiptLineItem['field_title'] = CRM_Utils_Array::value('label', $priceField);
                $skiptLineItem['description'] = '';
                $skiptLineItem['qty'] =  1;
                $skiptLineItem['unit_price'] = CRM_Utils_Array::value('amount', $value);
                $skiptLineItem['line_total'] = CRM_Utils_Array::value('amount', $value);
                $skiptLineItem['participant_count'] = 0;
                $skiptLineItem['max_value'] = NULL;
                $skiptLineItem['membership_type_id'] = NULL;
                $skiptLineItem['auto_renew'] = NULL;
                $skiptLineItem['html_type'] = CRM_Utils_Array::value('html_type', $priceField);
                $this->_lineItem[$participantNum] = array($priceFieldValueId => $skiptLineItem);

              }
            }
          }
          $params[$participantNum] = $skipParticipant;
        }
